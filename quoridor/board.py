...
class Board:
    ...
    def create_board(self):
        """
        Creates Board as 2d list.
        """
        self.board = [[Tile(TAN, 0, (j*TILE_WIDTH, MARGIN+i*TILE_HEIGHT)) for i in range(ROWS)] for j in range(ROWS)]  # Create empty ROWSxROWS board
        self.board[WHITE_START_POS[0]][WHITE_START_POS[1]].occupied = Pawn(WHITE, (WHITE_START_POS[0],WHITE_START_POS[1]))  # Create white piece at starting pos
        self.board[BLACK_START_POS[0]][BLACK_START_POS[1]].occupied = Pawn(BLACK, (BLACK_START_POS[0],BLACK_START_POS[1]))  # Create black piece at starting pos
    ...
    def can_place(self, wall, pos):
        ...
        x, y = pos
        if wall.dir == 1:
            if self.board[x][y].walls[0] or self.board[x][y+1].walls[0]:
                return False
            if self.board[x-1][y+1].walls[1] and self.board[x][y+1].walls[1]:
                return False
        else:
            if self.board[x][y].walls[1] or self.board[x+1][y].walls[1]:
                return False
            if self.board[x+1][y-1].walls[0] and self.board[x+1][y].walls[0]:
                return False
        return True
            ...
            for j in i:
                if j.occupied:
                    if j.occupied.color == WHITE:
                        lst[0] = j.occupied.pos
                    else:
                        lst[1] = j.occupied.pos
        return lst
