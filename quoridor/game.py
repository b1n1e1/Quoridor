...
class Game:
    ...
    def _init(self):
        ...
        self.valid_moves = []
        self.player1Walls = [Wall((0, 0), 1, GREEN) for _ in range(10)]
    ...
    def can_place(self, wall, pos):
        ...
        x, y = pos
        if wall.dir == 1:
            if self.board.board[x][y].walls[0] or self.board.board[x][y+1].walls[0]:
                return False
            if self.board.board[x-1][y+1].walls[1] and self.board.board[x][y+1].walls[1]:
                return False
        else:
            if self.board.board[x][y].walls[1] or self.board.board[x+1][y].walls[1]:
                return False
            if self.board.board[x+1][y-1].walls[0] and self.board.board[x+1][y].walls[0]:
                return False
        return True
